...
export const isFeedbackStartRequest = (text: string): boolean => {
  const normalized = text.trim().toLowerCase();
  return normalized === '💬 оставить обратную связь' || normalized === '/feedback';
};

export const isFeedbackCancelRequest = (text: string): boolean => {
  const normalized = text.trim().toLowerCase();
...
export const buildFeedbackInputKeyboard = (): {
  keyboard: Array<Array<{ text: string }>>;
  resize_keyboard: boolean;
} => ({
...
export const renderHowBotWorksRu = (): string =>
  [
    'Твой бот личный аналитик-саморазвития, который:',
    '- принимает твои мысли, эмоции, идеи, факты в течение дня,',
    '- структурирует их автоматически,',
    '- показывает повторяющиеся паттерны,',
    '- анализирует связи,',
    '- и каждое утро присылает глубокое, но чёткое summary вчерашнего дня.',
    '',
    'Это умный и понимающий помощник, который работает для тебя в режиме, который ты выбираешь.'
  ].join('\n');
...
export const renderFeedbackPromptRu = (): string =>
  [
